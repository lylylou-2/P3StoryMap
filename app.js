...
modal.addEventListener('close', function () {
    ...
    if (modal.returnValue === 'oui') {
        ...
        ajoutMarkerSurLaMap(inputTitre.value, inputDate.value, inputRaconte.value, coordonnée);
    }
});

// on charge les marqueurs du localstorage
for (var i = 0; i < tableauMarker.length; i++) {
    ajoutMarkerSurLaMap(tableauMarker[i].titre, tableauMarker[i].date, tableauMarker[i].raconte, tableauMarker[i].coordonnée);
}

function ajoutMarkerSurLaMap(titre, date, raconte, coordonnée) {
